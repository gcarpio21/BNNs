...
\headerbox{The libraries}{name=2ddiag, span=3, below=vmeas, above=foottext, column=0}{
  \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
    ...
    \begin{itemize}[noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt, leftmargin=12pt]
      ...
      \hfill
      \begin{minipage}[t]{0.48\linewidth}\centering
        $\mathcal{L}_{AvUC} = \dfrac{n_{AU}}{n_{AU}+n_{AC}} + \dfrac{n_{IC}}{n_{IC}+n_{IU}}$
      \end{minipage}
      \end{center}
    ...
    \begin{itemize}[noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt, leftmargin=12pt]
      \item \textbf{Scope}: last layer or all weights.
      \item \textbf{Hessian}: full, diagonal, or Kronecker-factored (\texttt{kron}).
      \item \textbf{Prior tuning}: optimise prior precision via marginal likelihood.
      \item \textbf{Usage}: \parbox[t]{\linewidth}{\ttfamily\small
