...
}
{
  ...
  \put(2.0,0.75){\includegraphics[height=50pt]{logos/face.png}}
...
\headerbox{Motivation}{name=motivation, column=0, row=0, span=3}{
  \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
   ...
   \item These get trained in a \textbf{frequentist} manner, which means that the model parameters are fixed point estimates after training. 
   \item \textbf{Bayesian Neural Networks (BNNs)} provide a probabilistic framework for neural networks, allowing for \textbf{uncertainty estimation} in predictions.
...
\node[bweight, right=1.5cm of BX1, label=above:{\small${\sim}p(w_1|D)$}] (BW1) {$w_1$};
\node[bweight, right=1.5cm of BXn, label=below:{\small${\sim}p(w_n|D)$}] (BWn) {$w_n$};
\node[op, right=2.5cm of Bdots] (BSum) {$\sum$};
\node[op, right=1.5cm of BSum] (BPlus) {$+$};
\node[bweight, above=1.5cm of BPlus, label=above:{\small${\sim}p(b|D)$}] (BB) {$b$};
...
\headerbox{Approximating the Posterior}{name=vmeas, span=3, below=motivation, column=0}{
  \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
    \item Objective: approximate the posterior $p(\theta|D)$. Two approaches via \textbf{\texttt{bayesian-torch}} and \textbf{\texttt{laplace-torch}}.
    \item \textbf{Variational Inference}: casts inference as optimization; minimizes KL divergence between $q(\theta)$ and the true posterior.
    \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=12pt]
      \item \textbf{Bayes by Backprop}: uses the reparameterization trick to allow gradients to flow through stochastic weights. Instead of sampling $w \sim \mathcal{N}(\mu, \sigma^2)$ directly, write
      ...
      $\hat{\mathbf{W}}_n = \mathbf{W}_{\mathrm{MAP}} + \mathbf{s}_n \odot \Delta\hat{\mathbf{W}} \odot \mathbf{r}_n^\top$
    \end{itemize}
    \item \textbf{Laplace Approximation}: fits a Gaussian to the posterior \emph{post-hoc} using the Maximum A Posteriori (MAP) estimate $\theta_{\mathrm{MAP}}$:
    \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=12pt]
...
%
%
\headerbox{The libraries}{name=2ddiag, span=3, below=vmeas, above=foottext, column=0}{
  \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
    \item \textbf{\texttt{bayesian-torch}}: converts deterministic layers to Bayesian via \texttt{dnn\_to\_bnn}.
    \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=12pt]
      ...
      \begin{minipage}[t]{0.48\linewidth}\centering
        $\mathcal{L}_\text{ELBO} = \mathbb{E}_{q}[\log p(D|\theta)] - \mathrm{KL}[q(\theta)\,\|\,p(\theta)]$
      \end{minipage}%
      \hfill
      \begin{minipage}[t]{0.48\linewidth}\centering
    ...
    \end{itemize}
    \item \textbf{\texttt{laplace-torch}}: post-hoc wrapper around any trained PyTorch model.
    \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=12pt]
      ...
      \item \textbf{Prior tuning}: optimise prior precision via marginal likelihood or grid search.
      \item \textbf{Usage}: \parbox[t]{\linewidth}{\ttfamily\small
...
\headerbox{Experiments}{name=resolution, span=3, row=0, column=3}{
  Implemented as Jupyter notebooks. Three datasets:
  \begin{itemize}[noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
    \item \textbf{Classification: Two Moons}: 2D toy dataset.
    \item \textbf{Regression: PovertyMap}: satellite imagery dataset for wealth index prediction.
    \item \textbf{Regression: Sinusoidal function}: 1D toy dataset.
  \end{itemize}
  \vspace{2pt}
  \begin{center}
  \begin{minipage}[t]{0.26\linewidth}\centering
    \includegraphics[height=4cm,width=\linewidth,keepaspectratio]{figures/two_moons_dataset.png}
  \end{minipage}\hfill
  \begin{minipage}[t]{0.26\linewidth}\centering
  ...
  \begin{minipage}[t]{0.44\linewidth}\centering
    \includegraphics[height=4cm,width=\linewidth,keepaspectratio]{figures/sinusoid_dataset.png}
  \end{minipage}
  \end{center}
  \vspace{2pt}
  ...
  \begin{tabular}{llrrr}
    \toprule
    \textbf{Dataset} & \textbf{Method} & \textbf{MAP/Fit (s)} & \textbf{LA Tune (s)} & \textbf{Inference (s)} \\
    \midrule
    \multirow{3}{*}{Two Moons}  & Flipout / scratch / ELBO  & $32.8 \pm 11.0$   & ---           & $0.047 \pm 0.001$ \\
                                & Flipout / scratch / AvUC  & $754.5 \pm 224.3$ & ---           & $0.047 \pm 0.002$ \\
                                & Laplace Kron / last       & $0.5 \pm 0.1$     & $2.8 \pm 0.7$ & $0.001 \pm 0.000$ \\
    \midrule
    \multirow{2}{*}{Sinusoidal} & Flipout / scratch / ELBO  & $160.3 \pm 97.6$  & ---           & $2.52 \pm 2.00$ \\
                                & Laplace Kron / last       & $0.2 \pm 0.1$     & $2.1 \pm 0.4$ & $0.107 \pm 0.025$ \\
    \bottomrule
  \end{tabular}}
}		
%
...
\headerbox{Summary and Outlook}{name=summary, span=3, below=resolution, above=foottext, column=3}{
  ...
  \begin{itemize}[noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
    \item Good calibration can be achieved.
    \item VI needs more training time.
    \item Laplace has good performance OOD.
  \end{itemize}
  \vspace{3pt}
  \textbf{Outlook}
  \begin{itemize}[noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt, leftmargin=10pt]
    \item Is ID vs.\ OOD performance usable as-is, or with minimal changes?
    \item VI: explore MOPED $\delta$ tuning.
    \item Laplace: Gaussian processes and low-rank Hessian structures unexplored.
    \item Compare to other implementations, e.g. Markov Chain Monte Carlo (MCMC) methods.
  \end{itemize}
}
%
%
