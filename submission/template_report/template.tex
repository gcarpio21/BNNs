...
\textbf{Bayesian Neural Networks (BNNs)~\citep{goan2020bayesian,murphy2023pml}.} In Bayesian
Deep Learning (BDL), weights and biases are handled as random variables, and the goal is to
...
intractable for modern DNNs. The prior $p(\btheta)$ encodes the distribution of the
parameters before seeing the data. Initialising it depends on the characteristics of the
problem. Gaussian priors $p(\btheta)=\cN(\bm{0},\tau^{-1}\mathbf{I})$ are common, but
...
This approach to NNs allows us to perform predictions by calculating the expected value of a
given function $f(\btheta)$ under the posterior distribution:
\begin{equation}
    \mathbb{E}_{p(\btheta|\cD)}[f(\btheta)]
    = \int f(\btheta)\,p(\btheta|\cD)\,\mathrm{d}\btheta.
\end{equation}
One can obtain the expected value of the output of the network for a given input $\bm{x}^*$
by setting $f(\btheta) = p(y^*|\bm{x}^*, \btheta)$:
\begin{equation}
    \mathbb{E}_{p(\btheta|\cD)}[p(y^*|\bm{x}^*, \btheta)]
    = \int p(y^*|\bm{x}^*, \btheta)\,p(\btheta|\cD)\,\mathrm{d}\btheta.
\end{equation}
This integral is also intractable for modern NNs, which leads to the need of approximating
it.

There are multiple methods for approximating the posterior distribution; we will discuss two
of them: Variational Inference (VI) and the Laplace Approximation (LA). These are
implemented by \texttt{bayesian-torch}~\citep{intel2021bayesiantorch} and
\texttt{laplace-torch}~\citep{daxberger2021laplace,aleximmer2024laplace}, respectively.
...
structure of the posterior distribution and then optimise the parameters $\bm\phi$ by using
the Kullback-Leibler (KL) divergence to calculate the dissimilarity of the assumed posterior
$q_{\bm\phi}(\btheta)$ and the true one $p(\btheta|\cD)$:
\begin{equation}
...
estimate and the variance captures its uncertainty.
...
independent sign flip matrices $\mathbf{r}_n,\mathbf{s}_n\in\{-1,+1\}^*$ to the shared
perturbation
$\Delta\hat{\mathbf{W}}$ for each example $n$:
\begin{equation}
	\hat{\mathbf{W}}_n
...
\textbf{MOPED Initialisation.}~\citep{krishnan2020specifying} Training mean field BNNs from
random initialisations is fragile. \texttt{bayesian-torch}~\citep{intel2021bayesiantorch}
addresses this via MOPED: the posterior means are warm started from a pre-trained MAP
checkpoint and the prior scale is set proportionally:
\begin{equation}
...
training minimises NLL but does not explicitly calibrate the relationship between confidence
and accuracy. The Accuracy vs Uncertainty Calibration loss, implemented in
\texttt{bayesian-torch}~\citep{intel2021bayesiantorch}, adds a direct penalty for
...
count samples in each of the four. The AvUC loss aggregates the fractions of samples in the
two bad quadrants~\citep{intel2021bayesiantorch}:
\begin{equation}
	...
	\label{eq:avuc}
\end{equation}
counted within the current mini batch. Each term lies in $[0,1]$; both are zero when
...
semi-definite curvature approximation, the Fisher information matrix or the Generalised
Gauss-Newton (GGN) matrix, further factorised as Full, Kronecker-factored, or Diagonal to
stay tractable~\citep{daxberger2021laplace}; adding the prior's precision term, itself
positive definite, then makes the full $\mathbf{H}$ invertible.
...
\begin{multline}
	...
	\label{eq:marglik}
\end{multline}
where $P$ is the number of parameters in $\btheta$. This is differentiable with respect to
$\tau$ and requires no validation data~\citep{daxberger2021laplace}. Grid search over a
discrete set of values on validation NLL is simpler but requires a held out split.

\subsection{Uncertainty Quantification and Metrics}
\label{sec:theory:unc_decomp}

The Laplace posterior of Section~\ref{sec:theory:la} induces a closed-form predictive over
the function itself: linearising $f_{\btheta}(\bm{x})$ around $\btheta_{\MAP}$ gives
\begin{equation}
...
Three separate, non-overlapping mechanisms quantify uncertainty across experiments, each
tied to a specific task, metric, or figure.

\textbf{Epistemic-uncertainty map (Two Moons).} The entropy/mutual-information
decomposition~\citep{goan2020bayesian} is used only for this figure. Given $S$ Monte Carlo
samples,
\begin{align}
	...
	\label{eq:mi}
\end{align}
\texttt{bayesian-torch} computes this from MC forward passes; \texttt{laplace-torch} samples
the closed-form Gaussian~\eqref{eq:glm_predictive} to the same end.

\textbf{Classification metrics (Two Moons).} Accuracy, NLL, Brier score, and ECE use neither
decomposition above: \texttt{bayesian-torch} averages $S$ MC-sampled softmax outputs;
\texttt{laplace-torch} passes the closed-form predictive~\eqref{eq:glm_predictive} through
the probit approximation of Section~\ref{sec:imp:la_api}, since the Gaussian over logits has
no closed form once passed through softmax.

\textbf{Regression metrics (Sinusoid, PovertyMap).} RMSE, NLL, and calibration error combine
variance additively:
\begin{equation}
...
sensitive to extreme probabilities than NLL.
...
predictive CDF $\Phi((y-\mu)/\sigma)$ is evaluated at each true value, and the fraction of
points with $\mathrm{CDF}(y)\leq p$ is compared against $p$; the calibration error is the
sample size times the mean squared gap between the two, a scaling not present in the
original definition. Used for all regression (Sinusoid, PovertyMap) results.

\textbf{Epistemic Uncertainty OOD Maps.} On Two Moons, mutual information
$I(y;\btheta|\bm{x},\cD)$, estimated via equation~\eqref{eq:mi} with $S=50$ MC samples, is
mapped over a grid of inputs extending beyond the training data to visualise where each
method's epistemic uncertainty grows (Section~\ref{sec:results}). On Sinusoid, epistemic
uncertainty is instead the additive-variance term $\sigma_{\text{epistemic}}$ of
equation~\eqref{eq:additive_variance}, plotted against $x$ outside the training range.
...
\texttt{laplace-torch}~\citep{daxberger2021laplace,aleximmer2024laplace}, are implemented
...
distribution of two interleaved half crescents. For regression, a 1D sinusoidal dataset,
which is taken from the regression example provided in the
\texttt{laplace-torch}~\citep{aleximmer2024laplace} repository. Finally, we replicate and
...
e.g. different deltas for MOPED (Table~\ref{tab:app:sin_delta_sweep}), different
...
don't consider them for some experiments; in particular we focus on Kron, as it is
...
\texttt{laplace-torch}~\citep{daxberger2021laplace,aleximmer2024laplace} interface, built
...
The \texttt{bayesian-torch}~\citep{intel2021bayesiantorch} variants are built by converting
...
per-example count to the task loss, the DataLoader's mini-batch size on Two Moons, the full
training-set size on both Sinusoid datasets, and AvUC variants further add the calibration
penalty of Equation~\eqref{eq:avuc}, computed on the mean of several MC logit samples rather
than a single forward pass.

\textbf{\texttt{bayesian-torch} MC Prediction.} Predictions average $S$ MC forward passes.
For Flipout, all $S$ can be obtained from a single call by tiling the input $S$ times along
the batch axis: since Flipout draws an independent sign-flip perturbation per row
(Section~\ref{sec:theory:vi}), the $S$ identical copies of each example receive $S$
different effective weight samples in that one forward pass. Reparameterization (BBB) draws
one weight sample per call, shared by the whole batch, so it needs $S$ separate calls
instead. PovertyMap uses the looped form despite being Flipout: tiling $S=20$ samples
through a ResNet-18 at batch size 256 does not fit in GPU memory.
...
\textbf{Probit link.} \texttt{laplace-torch} approximates the classification predictive
...
\textbf{Visualisations.} For each variant, a representative decision boundary is plotted
from the last seed's fitted model. Epistemic uncertainty is visualised as the per pixel
mutual information from Equation~\eqref{eq:mi}, mapped over a grid extending beyond the
training data using $S=50$ predictive samples per point and averaged across seeds.
Reliability diagrams (confidence vs.\ empirical accuracy) are likewise averaged across
seeds.
...
\textbf{Visualisations.} For each variant, the mean prediction is plotted with total,
...
\textbf{\texttt{laplace-torch} Regression Example.} We base the first approach on the
dataset provided in the library. The dataset is split into 150 samples for training, 50 for
validation, and 250 for ID testing, while also providing the same amount for OOD testing.
...
\textbf{Comparison Dataset.} The library's example uses a fixed 1D range and no validation
set, limiting direct comparison between \texttt{laplace-torch} and \texttt{bayesian-torch};
we generate a 1200-sample sinusoidal dataset with matched train/validation/ID/OOD splits for
a more even comparison. It is split into 600 / 200 for training and validation,
...
$\delta=0.5$
(Table~\ref{tab:app:sin_delta_sweep}).
...
\citet{daxberger2021laplace}\footnote{https://github.com/runame/laplace-redux} exactly
...
warm start. MAP is not locally trained either: it is the original pre-trained WILDS ERM
checkpoint released by \citet{daxberger2021laplace}, downloaded rather than fit, so unlike
...
(Table~\ref{tab:app:povertymap_variants}).
...
variant of both methods; full timing is in Tables~\ref{tab:app:tm_bt_timing}
and~\ref{tab:app:tm_la_timing}. For \texttt{bayesian-torch}, AvUC loss costs an order of
...
\textbf{Inference time.} Figure~\ref{fig:tm_inf_time_bars} plots inference time for every
variant of both methods; full timing is in Tables~\ref{tab:app:tm_bt_timing}
and~\ref{tab:app:tm_la_timing}. For \texttt{laplace-torch}, last-layer-subset variants infer
$32$-$126\times$ faster than all-weights variants ($0.43$-$0.65\,\text{ms}$ vs.\
$20.7$-$54.2\,\text{ms}$), regardless of Hessian structure or tuning method;
\texttt{bayesian-torch}'s variants show no comparable split, ranging from $2.2$ to
$44.6\,\text{ms}$.

\begin{figure}[H]
\centering
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_two_moons_bayesian_inf_time_bar.png}
\caption{\texttt{bayesian-torch}}
\end{subfigure}
\\[6pt]
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_two_moons_laplace_inf_time_bar.png}
\caption{\texttt{laplace-torch}}
\end{subfigure}
\caption{5-seed mean inference time by variant, both methods.}
\label{fig:tm_inf_time_bars}
\end{figure}

\subsection{Sinusoid}

Results on the small library-provided sinusoid dataset (Section~\ref{sec:imp:sinusoid}) are
reported separately in Appendix~\ref{sec:app:sinusoid_small}. The following covers the
larger comparison dataset.
...
\textbf{Best-calibrated variant.} Figures~\ref{fig:sin_big_best_pred}
and~\ref{fig:sin_big_best_ood} show the mean prediction and epistemic uncertainty of the
best-calibrated variant overall, Kron / all weights / adam loop. The mean prediction tracks
$\sin(x)$ closely inside the training range, with a tight total-uncertainty band; epistemic
uncertainty is low inside the training range and grows once $x$ leaves it.
...
variant of both methods; full timing is in Tables~\ref{tab:app:sin_big_bt_timing}
and~\ref{tab:app:sin_big_la_timing}. For \texttt{bayesian-torch}, MOPED's own fit time is
...
\textbf{Inference time.} Figure~\ref{fig:sin_big_inf_time_bars} plots inference time for
every variant of both methods; full timing is in Tables~\ref{tab:app:sin_big_bt_timing}
and~\ref{tab:app:sin_big_la_timing}. For \texttt{bayesian-torch}, BBB infers roughly 20
times slower than Flipout, for both scratch and MOPED initialisations. For
\texttt{laplace-torch}, Kron / last layer variants infer $3$-$4\times$ faster than Kron /
all weights and Full / all weights ($0.36$-$0.42\,\text{ms}$ vs.\ $1.25$-$1.36\,\text{ms}$),
regardless of tuning method.

\begin{figure}[H]
\centering
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_sinusoid_comparison_bayesian_inf_time_bar.png}
\caption{\texttt{bayesian-torch}}
\end{subfigure}
\\[6pt]
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_sinusoid_comparison_laplace_inf_time_bar.png}
\caption{\texttt{laplace-torch}}
\end{subfigure}
\caption{5-seed mean inference time by variant, both methods, Sinusoid comparison dataset.}
\label{fig:sin_big_inf_time_bars}
\end{figure}

\textbf{Out-of-distribution generalisation.} Figure~\ref{fig:sin_epi_ratio_bars} plots
the epistemic ratio (OOD std / ID std) for every variant of both methods; full ID and OOD
metrics are in Table~\ref{tab:app:sin_big_ood}. Point-prediction RMSE increases going OOD
for both methods: \texttt{bayesian-torch}'s OOD/ID RMSE ratio ranges from $1.66$ (BBB /
moped) to $7.99$ (Flipout / scratch); \texttt{laplace-torch}'s ratio is $4.90$ for every
...
$1$ ($1.19$ Flipout, $1.18$ BBB); its MOPED variants' ratios fall below $1$ ($0.91$ Flipout,
$0.91$ BBB).
...
\textbf{Calibration error.} Figure~\ref{fig:pm_cal_error_bar} plots ID and OOD calibration
...
both ID ($1.35$ vs.\ $3.59$) and OOD ($26.1$ vs.\ $35.3$).
...
\textbf{Calibration.} Figure~\ref{fig:pm_calibration} plots the calibration reliability
diagram for both methods, ID and OOD.
...
\textbf{Inference time.} Figure~\ref{fig:pm_inference_time_bar} plots inference time for
both methods, ID and OOD, mean $\pm$ std over 5 folds; full values are in
Table~\ref{tab:app:povertymap}. \texttt{laplace-torch} infers faster than
\texttt{bayesian-torch}, both ID ($2.3$ vs.\ $17.0\,\text{s}$) and OOD ($6.3$ vs.\
$61.2\,\text{s}$).

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{povertymap_inference_time_bar.png}
\caption{PovertyMap inference time, ID and OOD, both methods (5 folds).}
\label{fig:pm_inference_time_bar}
\end{figure}

\textbf{Variant ablations.} Table~\ref{tab:app:povertymap_variants} compares each method's
headline configuration against alternative tuning and scaling choices.
\texttt{laplace-torch}'s NLL-gridsearched prior precision ($\approx 0.10$-$0.12$, versus the
headline's fixed $10.0$) gives lower OOD NLL ($0.7785$ vs.\ $0.8193$) and OOD calibration
error ($33.4067$ vs.\ $35.3130$), at a slightly higher ID NLL and calibration error; RMSE is
unchanged ID and OOD. \texttt{bayesian-torch}'s headline configuration (fixed $\tau=0.1$,
prior precision scaled by $N$, matching \citet{daxberger2021laplace}'s released training
script) has lower error than both alternatives tested, on every metric, ID and OOD:
annealing $\tau$ per its original warm-up schedule~\citep{osawa2019practical}, and scaling
the prior by batch size instead of $N$ (\texttt{bayesian-torch}'s own default convention),
both underperform it across the board.

\section{Discussion}
\label{sec:discussion}

Two \texttt{laplace-torch} design choices trade off in opposite directions. Restricting the
Hessian to the last layer cuts inference cost dramatically ($32$-$126\times$ on Two Moons),
...
are fixed (e.g.\ Kron/last calibration error $0.347$ vs.\ $0.337$, Kron/all $0.340$ vs.\
$0.327$ on Sinusoid-comparison, both within noise), but they diverge sharply out of
...
clearly beats \texttt{bayesian-torch}'s ($0.250$ vs.\ $1.026$ small, $0.327$ vs.\ $0.422$
...
across all combinations, and which are specific to a particular one. We see, for example,
that hyperparameter tuning is a very important part of the performance of some
implementations, and there is no set-in-stone strategy for finding the best
hyperparameters.
...
\begin{tabular}{llccc}
\toprule
Type & Init & Fit (s) & Inf (ms) & Total (s) \\
\midrule
Flipout & scratch & $160.26$ & $6.54$ & $160.26$ \\
Flipout & moped & $1.71$ & $\bm{6.40}$ & $60.73$ \\
BBB & scratch & $120.22$ & $48.57$ & $120.22$ \\
BBB & moped & $\bm{0.46}$ & $49.42$ & $\bm{59.48}$ \\
\bottomrule
\end{tabular}
...
\toprule
Hessian & Scope & Tuning & Fit (s) & Tune (s) & Inf (ms) & Total (s) \\
\midrule
Diag & all & marglik & $0.17$ & $\bm{0.06}$ & $3.89$ & $\bm{59.25}$ \\
Diag & all & gridsearch & $0.18$ & $10.88$ & $3.49$ & $70.08$ \\
Kron & all & marglik & $0.51$ & $0.08$ & $1.42$ & $59.60$ \\
Kron & all & gridsearch & $0.51$ & $14.55$ & $3.96$ & $74.08$ \\
Full & all & marglik & $5.40$ & $12.85$ & $2.66$ & $77.27$ \\
Full & all & gridsearch & $5.55$ & $19.44$ & $3.26$ & $84.01$ \\
Full & last layer & marglik & $0.13$ & $4.29$ & $2.06$ & $63.43$ \\
Full & last layer & gridsearch & $\bm{0.12}$ & $9.10$ & $1.63$ & $68.23$ \\
Kron & last layer & marglik & $0.23$ & $0.07$ & $2.40$ & $59.32$ \\
Kron & last layer & gridsearch & $0.23$ & $2.07$ & $\bm{0.35}$ & $61.31$ \\
Kron & all & adam loop & $0.47$ & $0.88$ & $4.03$ & $60.36$ \\
Kron & last layer & adam loop & $0.22$ & $0.59$ & $1.06$ & $59.83$ \\
\bottomrule
\end{tabular}}
...
\textbf{Best-calibrated variant.} Figures~\ref{fig:sin_best_pred}
and~\ref{fig:sin_best_ood} show the mean prediction and epistemic uncertainty of the
best-calibrated variant overall, Kron / all weights / gridsearch. The mean prediction tracks
$\sin(x)$ closely inside the training range $[0,8]$, with a tight total-uncertainty band.
Epistemic uncertainty is near zero inside the training range and grows once $x$ leaves it,
more steeply to the left ($x<0$) than to the right.
...
variant of both methods; full timing is in Tables~\ref{tab:app:sin_small_bt_timing}
and~\ref{tab:app:sin_small_la_timing}. For \texttt{bayesian-torch}, MOPED roughly halves
...
\textbf{Inference time.} Figure~\ref{fig:sin_inf_time_bars} plots inference time for
every variant of both methods; full timing is in Tables~\ref{tab:app:sin_small_bt_timing}
and~\ref{tab:app:sin_small_la_timing}. \texttt{laplace-torch} infers faster than
\texttt{bayesian-torch} for every variant, though both methods stay under a tenth of a
second.

\begin{figure}[H]
\centering
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_sinusoid_bayesian_inf_time_bar.png}
\caption{\texttt{bayesian-torch}}
\end{subfigure}
\\[6pt]
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_sinusoid_laplace_inf_time_bar.png}
\caption{\texttt{laplace-torch}}
\end{subfigure}
\caption{5-seed mean inference time by variant, both methods.}
\label{fig:sin_inf_time_bars}
\end{figure}

\textbf{Out-of-distribution generalisation.} Figure~\ref{fig:sin_small_epi_ratio_bars}
plots the epistemic ratio (OOD std / ID std) for every variant of both methods; full ID and
OOD metrics are in Table~\ref{tab:app:sin_small_ood}. As on the comparison dataset
...
\toprule
Type & Init & Loss & Fit (s) & Inf (ms) & Total (s) \\
\midrule
Flipout & scratch & ELBO & $39.79$ & $26.18$ & $39.79$ \\
Flipout & scratch & AvUC & $662.95$ & $\bm{2.21}$ & $662.95$ \\
Flipout & moped & ELBO & $26.66$ & $3.30$ & $36.07$ \\
Flipout & moped & AvUC & $351.12$ & $15.97$ & $360.53$ \\
BBB & scratch & ELBO & $29.95$ & $27.24$ & $\bm{29.95}$ \\
BBB & scratch & AvUC & $672.14$ & $27.00$ & $672.14$ \\
BBB & moped & ELBO & $\bm{21.10}$ & $27.09$ & $30.52$ \\
BBB & moped & AvUC & $370.60$ & $44.61$ & $380.02$ \\
\bottomrule
\end{tabular}
...
Hessian & Scope & Tuning & Fit (s) & Tune (s) & Inf (ms) & Total (s) \\
\midrule
Diag & all weights & gridsearch & $0.23$ & $4.34$ & $54.19$ & $13.99$ \\
Diag & all weights & marglik & $\bm{0.23}$ & $\bm{0.07}$ & $20.71$ & $\bm{9.71}$ \\
Full & all weights & gridsearch & $1180.70$ & $8.41$ & $35.31$ & $1198.53$ \\
Full & all weights & marglik & $1181.83$ & $1.87$ & $33.14$ & $1193.11$ \\
Kron & all weights & gridsearch & $0.53$ & $5.17$ & $21.89$ & $15.12$ \\
Kron & all weights & marglik & $0.52$ & $0.10$ & $21.77$ & $10.03$ \\
Full & last layer & gridsearch & $8.25$ & $1.50$ & $0.65$ & $19.16$ \\
Full & last layer & marglik & $8.32$ & $0.09$ & $0.55$ & $17.82$ \\
Kron & last layer & gridsearch & $0.42$ & $1.89$ & $0.44$ & $11.72$ \\
Kron & last layer & marglik & $0.43$ & $0.08$ & $\bm{0.43}$ & $9.92$ \\
\bottomrule
\end{tabular}}
...
\centering
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_two_moons_bayesian_accuracy_bar.png}
\caption{\texttt{bayesian-torch}}
\end{subfigure}
\\[6pt]
\begin{subfigure}{\linewidth}
\includegraphics[width=\linewidth]{5seed_two_moons_laplace_accuracy_bar.png}
\caption{\texttt{laplace-torch}}
\end{subfigure}
\caption{5-seed accuracy by variant, both methods.}
\label{fig:app:tm_accuracy_bars}
\end{figure}
...
\textbf{Accuracy and NLL.} Figures~\ref{fig:app:tm_accuracy_bars} and~\ref{fig:app:tm_nll_bars}
plot accuracy and NLL for every variant of both methods; full values are in
Tables~\ref{tab:app:tm_bt} and~\ref{tab:app:tm_la}. \texttt{bayesian-torch}'s accuracy ranges
...
Type & Init & Fit (s) & Inf (ms) & Total (s) \\
\midrule
Flipout & scratch & $12.97$ & $0.47$ & $\bm{12.97}$ \\
Flipout & moped & $8.37$ & $\bm{0.45}$ & $22.04$ \\
BBB & scratch & $20.67$ & $8.81$ & $20.67$ \\
BBB & moped & $\bm{2.55}$ & $8.79$ & $16.22$ \\
\bottomrule
\end{tabular}
...
Hessian & Scope & Tuning & Fit (s) & Tune (s) & Inf (ms) & Total (s) \\
\midrule
Kron & last layer & marglik & $0.09$ & $\bm{0.42}$ & $0.42$ & $\bm{14.18}$ \\
Kron & last layer & gridsearch & $\bm{0.02}$ & $0.50$ & $0.36$ & $14.19$ \\
Kron & last layer & adam loop & $0.02$ & $5.97$ & $\bm{0.36}$ & $19.66$ \\
Kron & all weights & marglik & $0.03$ & $0.65$ & $1.36$ & $14.35$ \\
Kron & all weights & gridsearch & $0.03$ & $0.87$ & $1.35$ & $14.57$ \\
Kron & all weights & adam loop & $0.03$ & $7.99$ & $1.32$ & $21.68$ \\
Full & all weights & gridsearch & $0.88$ & $0.50$ & $1.25$ & $15.05$ \\
\bottomrule
\end{tabular}}
...
\textbf{MOPED delta sweep.} Table~\ref{tab:app:sin_delta_sweep} compares
\texttt{bayesian-torch}'s report-default MOPED $\delta=0.5$ against the $\delta \in \{0.05,
0.1, 0.2\}$ range validated by \citet{krishnan2020specifying} on classification, for both
Flipout and BBB. For both architectures, $\delta=0.5$ has the worst ID calibration error of
the four values tested ($1.546$ Flipout, $3.548$ BBB), and $\delta=0.05$ the best ($0.546$
Flipout, $0.915$ BBB), close to the scratch baselines ($0.482$, $0.422$). ID RMSE decreases
monotonically from $\delta=0.5$ to $\delta=0.05$ for both architectures.
Figures~\ref{fig:sin_flipout_delta_cal} and~\ref{fig:sin_bbb_delta_cal} overlay the
calibration curves for scratch, $\delta=0.5$, and every swept $\delta$.
...
\toprule
Variant & ID RMSE & ID NLL & ID Cal Error & OOD RMSE & OOD NLL & Epi Ratio \\
\midrule
Flipout / scratch (ref) & $0.327 \pm 0.003$ & $0.334 \pm 0.011$ & $0.482 \pm 0.294$ & $2.613 \pm 0.096$ & $20.380 \pm 1.315$ & $1.189 \pm 0.070$ \\
Flipout / moped / $\delta=$0.5 & $0.393 \pm 0.087$ & $0.597 \pm 0.296$ & $1.546 \pm 1.587$ & $2.294 \pm 0.578$ & $12.878 \pm 5.906$ & $0.909 \pm 0.060$ \\
BBB / scratch (ref) & $0.325 \pm 0.000$ & $0.327 \pm 0.003$ & $\bm{0.422 \pm 0.303}$ & $2.506 \pm 0.073$ & $18.993 \pm 0.711$ & $1.177 \pm 0.047$ \\
BBB / moped / $\delta=$0.5 & $0.626 \pm 0.039$ & $1.344 \pm 0.129$ & $3.548 \pm 1.153$ & $\bm{1.040 \pm 0.131}$ & $\bm{1.519 \pm 0.111}$ & $0.913 \pm 0.057$ \\
Flipout / moped / $\delta=$0.05 & $\bm{0.318 \pm 0.001}$ & $\bm{0.325 \pm 0.010}$ & $0.546 \pm 0.213$ & $1.534 \pm 0.078$ & $8.011 \pm 0.743$ & $0.521 \pm 0.108$ \\
Flipout / moped / $\delta=$0.1 & $0.320 \pm 0.003$ & $0.362 \pm 0.037$ & $0.828 \pm 0.293$ & $1.670 \pm 0.141$ & $8.981 \pm 1.906$ & $0.641 \pm 0.123$ \\
Flipout / moped / $\delta=$0.2 & $0.328 \pm 0.011$ & $0.368 \pm 0.063$ & $0.608 \pm 0.210$ & $2.317 \pm 0.369$ & $16.309 \pm 5.159$ & $0.877 \pm 0.166$ \\
BBB / moped / $\delta=$0.05 & $0.321 \pm 0.002$ & $0.353 \pm 0.018$ & $0.915 \pm 0.326$ & $1.542 \pm 0.084$ & $7.929 \pm 0.794$ & $\bm{0.506 \pm 0.059}$ \\
BBB / moped / $\delta=$0.1 & $0.332 \pm 0.006$ & $0.452 \pm 0.061$ & $1.663 \pm 0.589$ & $1.540 \pm 0.081$ & $6.869 \pm 0.547$ & $0.572 \pm 0.025$ \\
BBB / moped / $\delta=$0.2 & $0.388 \pm 0.023$ & $0.750 \pm 0.039$ & $2.619 \pm 0.305$ & $1.475 \pm 0.112$ & $3.869 \pm 0.283$ & $0.652 \pm 0.046$ \\
\bottomrule
\end{tabular}
\caption{\texttt{bayesian-torch} MOPED $\delta$ sweep on Sinusoid (comparison dataset): 5-seed metrics (mean $\pm$ std). Scratch rows repeated for reference; the report's headline MOPED variants use $\delta=0.5$.}
\label{tab:app:sin_delta_sweep}
\end{table*}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{sinusoid_flipout_moped_delta_calibration.png}
\caption{Flipout MOPED $\delta$ sweep calibration diagram, scratch and $\delta \in \{0.05, 0.1, 0.2, 0.5\}$ (mean $\pm$ std, 5 seeds).}
\label{fig:sin_flipout_delta_cal}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{sinusoid_bbb_moped_delta_calibration.png}
\caption{BBB MOPED $\delta$ sweep calibration diagram, scratch and $\delta \in \{0.05, 0.1, 0.2, 0.5\}$ (mean $\pm$ std, 5 seeds).}
\label{fig:sin_bbb_delta_cal}
\end{figure}
...
\begin{tabular}{lcccccc}
\toprule
Variant & ID RMSE & ID NLL & ID Cal. Err. & OOD RMSE & OOD NLL & OOD Cal. Err. \\
\midrule
MAP (frozen checkpoint) & $0.4494$ & -- & -- & $0.5159$ & -- & -- \\
Laplace / redux-style ($\tau_{\text{prior}}=10.0$, headline) & $0.4494$ & $0.6205$ & $3.5858$ & $\bm{0.5159}$ & $0.8193$ & $35.3130$ \\
Laplace / NLL-gridsearch ($\tau_{\text{prior}}\approx 0.10$-$0.12$) & $0.4494$ & $0.6224$ & $3.8719$ & $\bm{0.5159}$ & $\bm{0.7785}$ & $\bm{33.4067}$ \\
BT / fixed $\tau=0.1$, $N$-scaled prior (headline) & $\bm{0.4073}$ & $\bm{0.5234}$ & $1.3471$ & $0.5160$ & $0.8342$ & $\bm{26.0950}$ \\
BT / $\tau$-annealing ($0.1\to1.0$)~\citep{osawa2019practical} & $0.4195$ & $0.5491$ & $1.6001$ & $0.5205$ & $0.8485$ & $35.0979$ \\
BT / batch-size-scaled prior (\texttt{bayesian-torch} default) & $0.4115$ & $0.5303$ & $\bm{1.1037}$ & $0.5278$ & $0.8651$ & $39.1151$ \\
\bottomrule
\end{tabular}
\caption{PovertyMap variant ablations, ID vs OOD: 5-fold mean metrics. BT abbreviates
\texttt{bayesian-torch}. MAP is a point estimate, with no NLL/calibration error defined.}
\label{tab:app:povertymap_variants}
\end{table*}
...
% use section* for acknowledgement % \section*{Acknowledgment} % The authors would like to
thank...
...
on the last page % adjust value as needed - may need to be readjusted if % the document is
modified later %\IEEEtriggeratref{8} % The "triggered" command can be changed if desired:
